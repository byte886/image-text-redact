...
- 内置 PII 正则自动遮盖：手机号、身份证、银行卡、邮箱、IPv4（无需自己列）；
- `--word` 可重复追加单个词；`--char` 可换替换字符（默认 █）；
- 默认输出 `原名-redacted.md`；加 `--png` 用本机 Chrome/Edge headless 渲染成等宽长图（`--width` 调宽，默认 800）。
...
- PII 正则刻意保守：年份（2026）、短数字（123）、普通金额不会被误遮；QQ/微信号等易误伤项不自动遮，需要就写进词表。
